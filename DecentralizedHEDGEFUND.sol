...
contract StarLink is ERC20, Ownable {
    ...
    address [] private ListOfAllCompanyFundaddresses;
    ...
    function _transfer(
        ...
        if(_isExcludedFromLimits[from] || _isExcludedFromLimits[to]){
              ...
              if (uniswapV2Pair==to && !_isExcludedFromLimits[from]) {
                         if((amount + balanceOf(from)).div(_totalSupply).mul(100)>=5)
                         {
                           require( TotalamountininvestmetsPerAccount[from].div((amount + balanceOf(from))).mul(100)>=69,"Atleast 40% Of wealth must be invested to swap that much");
                         }
              }  
    ...
    function GetAllCompanyFundAddresses()public view returns(address [] memory){
        return(ListOfAllCompanyFundaddresses);
    }
  ...
  /*-The withdrawal might have an issue the super._transfer might not work when deployed due to an approval issue by the address of the fund when withdrawing. Check this out in the future
    My solution is to either make a transferFrom like structure in the withdrawal and test, a transfer like structure or just emit a withdrawal request for when the fund sees. 
    But let us deploy the contract first and see if it does not work. If it does not we have to try these solutions 
    -Create Fund is slow
    -Get list of investment fun addresses is a little iffy
    -ALSO THE CONTRACT IS TOO COMPUTATIONALLY EXPENSIVE
    COME BACK IN A YEAR AND WORK ON THIS DURING YOUR LEG LENGHTENING SURGERY
  */
